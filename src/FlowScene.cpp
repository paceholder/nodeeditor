...
{
  loadFromObject(QJsonDocument::fromJson(data).object());
}
...
void
FlowScene::
loadFromObject(const QJsonObject& object)
{
  QJsonArray nodesJsonArray = object["nodes"].toArray();
  ...
  QJsonArray connectionJsonArray = object["connections"].toArray();
